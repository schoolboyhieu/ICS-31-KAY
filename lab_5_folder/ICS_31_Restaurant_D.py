...
def handle_commands(C: list) -> list:
    ...
    while True:
        ...
        if response=="q":
            ...
            for name in C:
                C = Collection_remove_by_name(C, name.name)
        elif response == 'c':
            n = input("How much do you want to increase by: ")
...
def Collection_change_price(C: list, n: float) -> list:
    rl = []
    for x in C:
        new_price = Restaurant(x.name, x.cuisine, x.phone, x.dish,
                               Restaurant_change_price(x, n))
        rl.append(new_price)
    return rl
