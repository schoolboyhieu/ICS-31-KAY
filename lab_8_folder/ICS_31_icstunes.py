...
print(top_n_played(MUSIC, 3))
print()
print('E1--------------------')
print()

def Song_str(s: Song) -> str:
    '''Returns a str into any format when entered a song'''
    return 'Track : {}, Title : {}, Length : {}, Play Count : {} '.format(s.track,
                                                                          s.title, s.length, s.play_count)
print(Song_str(MUSIC[0].songs[0]))
print()

def Album_str(a: Album) -> str:
    '''Returns Album information more readable form'''
    newAlbum = ''
    for s in a.songs:
        newAlbum += Song_str(s) + '\n'
    return newAlbum
...
def Songdisplay_str(SD: Songdisplay) -> str:
    '''Returns information of Songdisplay more readable'''
    newdisplay = ''
    for s in SD:
        newdisplay += '\n' + 'Artist: {}, A_Title: {}, Year: {}, Track: {}, S_Title: {}, Length: {}, Play Count: {}'.format(s.artist, s.a_title,
                                                                                                                   s.year, s.track, s.s_title, s.length, s.play_count)
    return newdisplay
print(top_n_played(MUSIC, 5))
print()
print(Songdisplay_str(top_n_played(MUSIC, 5)))
print()
print('E2---------------------------')
print()

def Album_song_sort(A: Album)-> int:
    '''Returns the length of song list'''
    return len(A.songs)

MUSIC.sort(key=Album_song_sort)
print(MUSIC)
print()
print(Album_str(MUSIC[0]))
print()

def Album_name_sort(A: Album) -> int:
    '''Returns the length of Album name'''
    return len(A.artist)
collection_sort(MUSIC, Album_name_sort)
print(MUSIC)
print()
...
def unplayed_songs(l: list) -> list:
    '''Returns a list of Songdisplays'''
    displaylist = []
    Newdisplay = all_Songdisplays(l)
    for count in Newdisplay:
        if count.play_count == 0:
            displaylist.append(count)
    return displaylist
...
def length_from_songdisplay(SD: Songdisplay) -> int:
    '''Returns a list of length of songdisplay'''
    return SD.length
print()
print('E5-----------------------------')
print()

def total_time(A: Album) -> int:
    '''Returns the total time the user spent listening'''
    total = 0
    for S in A.songs:
        x = S.length * S.play_count
        total += x
    return total
def favorite_album(l: list) -> Album:
    '''Returns the favorite album from list of albums'''
    MC = []
    for A in l:
        MC.append(A)
    MC.sort(key=total_time,reverse=True)
    return MC[0]
...
def top_n(l: list, n: int, keyf: 'Function on Album', b: bool) -> 'list of Songdisplay':
    '''Returns a list of Songdisplay (more generalized form of top_n_played)'''
    Songdisplay_list = all_Songdisplays(l)
    Songdisplay_list.sort(key=keyf, reverse=b)
    return Songdisplay_list[:n]
print(top_n(MUSIC, 3, length_from_songdisplay, False))
print()
print('E7----------------------------')
print()

def longest_album_name(A: Album) -> int:
    '''Returns the length of Album's name'''
    return len(A.title)

def favorite_album2(l: list, keyf: 'Function on Album') -> Album:
    '''Returns an album that contains the most that was input for keyf'''
    MC = []
    for A in l:
        MC.append(A)
    MC.sort(key=keyf, reverse=True)
    return MC[0]
print(favorite_album2(MUSIC, longest_album_name))
print()
print('E8----------------------------')
print()

def collection_search(C: list, name: str) -> 'list of Songdisplays':
    '''Returns a list of Songdisplays when entered a list and word that may match in list'''
    s_display = all_Songdisplays(C)
    new_display = []
    for s in s_display:
        if name in s:
            new_display.append(s)
    return new_display
print(collection_search(MUSIC, 'Let It Bleed'))
